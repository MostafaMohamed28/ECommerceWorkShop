...
namespace ECommerce.Application.Services.OrderServices
{
    ...
    //    public OrderService(IOrderReadRepository orderReadRepository, ICustomerRepository customerRepository, IUnitOfWork unitOfWork, IProductReadRepository productReadRepository)
    //    {
    ...
    //    {
    //        var order = await _orderReadRepository.GetOrderByIdAsync(id, ct);
    //        if (order == null)
    //        {
    ...
    //        var orders = await _orderReadRepository.GetByCustomerIdAsync(customerId, ct);
